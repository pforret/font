...
show_usage() {
  ...
  out "Description: manage installed fonts for Linux and MacOS"
...
lookup_script_data() {
  ...
  Linux | GNU*)
    if [[ $(command -v lsb_release) ]]; then
      ...
      os_name=$(lsb_release -i | awk -F: '{ gsub(/\s+/,""); print $2}')    # Ubuntu
      os_version=$(lsb_release -r | awk -F: '{ gsub(/\s+/,""); print $2}') # 20.04
    else
      # Synology, QNAP,
      os_name="Linux"
    fi
